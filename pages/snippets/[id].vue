  ...
  <div class="dark:bg-slate-800 bg-white">
    <section class="container mx-auto px-4 py-6 max-w-5xl">
      <div class="flex flex-wrap -mx-4 mt-4">
        ...
        <div class="w-full px-4 mb-6" v-for="snippet in snippets" :key="snippet.id">
          <Hashtags :hashtags="snippet.hashtags" />
          <Title :title="snippet.title" />
          <div class="flex items-center justify-between mb-4">
            <div class="text-sm text-gray-600 dark:text-gray-400 flex space-x-4 mt-2">
              <div class="flex items-center justify-center">
                <img src="/blog/date.svg" alt="">
                <Date :date="snippet.create_at" />
              </div>
              <span class="mx-2 text-gray-600 dark:text-gray-400"> • </span>
              <div class="flex items-center">
                <img src="/blog/reading_time.svg" alt="">
                <ReadingTime :reading_time="snippet.reading_time" />
              </div>
              <span class="mx-2 text-gray-600 dark:text-gray-400"> • </span>
              <div class="flex items-center">
                <img src="/blog/views.svg" alt="">
                <span class="ml-1.5 md:ml-2 text-gray-600 dark:text-gray-400">- - - views</span>
              </div>
            </div>
          </div>
...
import Hashtags from "~/components/snippets/detail/header/Hashtags.vue";
import Title from "~/components/snippets/detail/header/Title.vue";
import Date from "~/components/snippets/detail/header/Date.vue";
import ReadingTime from "~/components/snippets/detail/header/ReadingTime.vue";
...
onMounted(() => {
  ...
  if (storedSnippets && storedRenderedContent) {
    snippets.value = JSON.parse(storedSnippets);
